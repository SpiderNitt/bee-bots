...
def followpath(path, objectHandle, port):
    ...
    if clientID != -1:
        ...
        _, lm = vrep.simxGetObjectHandle(
            clientID, "LeftJoint", vrep.simx_opmode_oneshot_wait
        )
        _, rm = vrep.simxGetObjectHandle(
            clientID, "RightJoint", vrep.simx_opmode_oneshot_wait
        )
        _, ebot = vrep.simxGetObjectHandle(
            clientID, "eBot", vrep.simx_opmode_oneshot_wait
        )
        _, picksubs = vrep.simxGetObjectHandle(
            clientID, "Cuboid14", vrep.simx_opmode_oneshot_wait
        )
